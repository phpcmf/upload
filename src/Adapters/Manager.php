...
class Manager
{
    ...
    {
        $s3Config = [
            'region'                  => empty($this->settings->get('cmf-upload.awsS3Region')) ? null : $this->settings->get('cmf-upload.awsS3Region'),
            'version'                 => 'latest',
            'endpoint'                => empty($this->settings->get('cmf-upload.awsS3Endpoint')) ? null : $this->settings->get('cmf-upload.awsS3Endpoint'),
            'use_path_style_endpoint' => empty($this->settings->get('cmf-upload.awsS3UsePathStyleEndpoint')) ? null : (bool) $this->settings->get('cmf-upload.awsS3UsePathStyleEndpoint'),
        ];

        // Only explicitly provide credentials if available.
        // Otherwise S3Client will attempt to use instance profile.
        if ($this->settings->get('cmf-upload.awsS3Key') && $this->settings->get('cmf-upload.awsS3Secret')) {
            $s3Config['credentials'] = [
                'key'    => $this->settings->get('cmf-upload.awsS3Key'),
                'secret' => $this->settings->get('cmf-upload.awsS3Secret'),
            ];
        }
        ...
        return new Adapters\AwsS3(
            new AwsS3Adapter(
                ...
                $this->settings->get('cmf-upload.awsS3Bucket')
            )
        );
    }
    ...
    {
        if (!$this->settings->get('cmf-upload.imgurClientId')) {
            return null;
        }
        ...
        return new Adapters\Imgur(
            new Guzzle([
                ...
                'headers'  => [
                    'Authorization' => 'Client-ID '.$this->settings->get('cmf-upload.imgurClientId'),
                ],
            ])
        );
    }
    ...
    {
        if (!$this->settings->get('cmf-upload.qiniuKey')) {
            return null;
        }

        $client = new QiniuAdapter(
            $this->settings->get('cmf-upload.qiniuKey'),
            $this->settings->get('cmf-upload.qiniuSecret'),
            $this->settings->get('cmf-upload.qiniuBucket'),
            $this->settings->get('cmf-upload.cdnUrl')
        );
